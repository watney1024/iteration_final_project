...
#include "gauss_seidel_3d_simd.h"
#include <iostream>
...
void run_test(int N, int max_iter, double tol, int threads, const string& method, ofstream& csv_out) {
    ...
    if (method == "Original") {
        ...
        GaussSeidel3DSIMD::init_test_problem(u, f, u_exact, N, h);
    }
    ...
    if (method == "Original") {
        GaussSeidel3D::solve_parallel_redblack(u, f, N, h, max_iter, tol, iter_count, residual, threads);
    } else if (method == "Tiled") {
        GaussSeidel3DTiled::solve_4level_tiling(u, f, N, h, max_iter, tol, iter_count, residual, threads);
    } else if (method == "SIMD") {
        GaussSeidel3DSIMD::solve_parallel_redblack_simd(u, f, N, h, max_iter, tol, iter_count, residual, threads);
    }
...
int main() {
    ...
    csv_out.close();
    cout << "\n结果已保存到 simd_results_3d.csv" << endl;
