...
| 对比维度 | 串行实现（conv.cpp） | 并行实现（conv_openmp.cpp） |
|:---------|:-------------------|:--------------------------|
| **索引方式** | 动态计数：`output[cnt[c]++]` | 静态计算：`i*oh*ow + h*ow + w` |
| **weight位置** | 循环累加：`weight_pos += k` | 公式计算：`i*pc*k + c*k` |
| **并行粒度** | 无并行 | 输出通道级（32通道） |
| **线程安全** | 单线程 | 多线程安全 |
| **内存访问** | 数组索引 | 数组索引 |
| **计算优化** | 无 | 无 |
...
2. **dim并行完全失败**：
   - 单线程性能正常（18.35ms）
   - 多线程反而变慢，20线程达22.11ms
   - **原因**：dim=1（batch维度），无并行度可挖掘
...
并行效率：5.77/20 = 28.85%
单线程性能：30.51/20 ≈ 1.53 GFLOPS
理论峰值（单核4.0GHz，8 FLOP/cycle）：32 GFLOPS
单核效率：1.53/32 ≈ 4.78%
...
2. **单核低效率（4.78%）**：
   ...
   - Padding数据（零填充）占用带宽但无计算贡献
   - 复杂索引计算（`i*oh*ow + h*ow + w`）消耗ALU周期
   - 权重访问模式非连续（跨通道跳跃访问）
