# 批量测试不同线程数的conv_openmp_new程序（优化版本）
...
# 编译程序
Write-Host "Compiling conv_openmp_new.cpp..." -ForegroundColor Yellow
g++ -fopenmp -O2 -std=c++11 conv_openmp_new.cpp -o conv_openmp_new.exe
...
# 创建结果文件
$resultFile = "conv_openmp_new_results.txt"
...
foreach ($threads in $threadCounts) {
    ...
    # 运行程序
    $output = .\conv_openmp_new.exe $threads
