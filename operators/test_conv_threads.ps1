# 批量测试不同线程数的conv_openmp程序
# 使用方法: .\test_conv_threads.ps1
...
Write-Host "Conv OpenMP Multi-Thread Performance Test" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# 编译程序
Write-Host "Compiling conv_openmp.cpp..." -ForegroundColor Yellow
g++ -fopenmp -O2 -std=c++11 conv_openmp.cpp -o conv_openmp.exe
...
# 创建结果文件
$resultFile = "conv_openmp_results.txt"
$timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
"Conv OpenMP Performance Test Results" | Out-File -FilePath $resultFile
...
foreach ($threads in $threadCounts) {
    ...
    # 运行程序
    $output = .\conv_openmp.exe $threads
