...
\begin{itemize}
    ...
    \item \textbf{矩阵乘法}：对于$n \times n$矩阵，算术强度约为$\frac{n}{3}$ FLOP/byte，$n$越大越显著
\end{itemize}
...
\begin{itemize}
    ...
    \item 主要挑战：负载均衡；
    \item 优化重点：增加计算并行度；
    \item 性能特征：接近Peak FLOPS（屋顶的水平部分）。
\end{itemize}
...
\begin{itemize}
    \item 并行加速比：受限；
    \item 主要挑战：内存带宽竞争；
    \item 优化重点：减少内存访问、提高缓存效率；
    \item 性能特征：受限于Memory Bandwidth（屋顶的斜线部分）。
\end{itemize}
...
\begin{itemize}
    \item $n < 1M$：\textbf{必须使用串行Thomas}，并行完全无收益（甚至负优化）
    \item $1M \leq n < 10M$：\textbf{RecursiveDoubling，16线程}
    \begin{itemize}
        \item 1M: 加速比1.43×（相对Sequential），推荐16线程
        \item 4M: 加速比2.03×（相对Sequential），推荐16线程
        \item 8M: 加速比2.28×（相对Sequential），推荐16线程
    \end{itemize}
    \item $10M \leq n < 100M$：\textbf{RecursiveDoubling，16-20线程}，加速比2.4-2.7×
    \item $n \geq 100M$：\textbf{考虑GPU加速}或分布式求解
\end{itemize}

\textbf{特别警告：}
\begin{itemize}
    \item ❌ 规模<1M使用并行会导致2-6倍性能下降
    \item ⚠️ RecursiveDoubling单线程比Sequential慢2.5×，必须使用≥8线程才有收益
    \item ⚠️ 超过16线程收益递减，20线程可能反而慢（如4M规模）
\end{itemize}
...
\begin{itemize}
    \item \textbf{依赖链难打破}：Thomas算法的$O(n)$依赖链需要复杂的递归策略才能并行化
    \item \textbf{规模阈值极高}：需要$n \geq 1M$才能获得有效加速（远高于Gauss-Seidel的512²）
    \item \textbf{加速比有限}：即使在32M规模、20线程，最佳加速比仅2.65×（并行效率13\%）
\end{itemize}

\textbf{为什么并行效率这么低？}

以4M问题、RecursiveDoubling、16线程为例（最佳配置，加速比2.03×）：

\textbf{时间分解分析：}

\begin{itemize}
    \item \textbf{理论最优时间}：46.98ms / 16 = 2.94ms（理想情况）
    \item \textbf{实际执行时间}：23.11ms（实测值）
    \item \textbf{并行损失}：23.11 - 2.94 = 20.17ms（\textbf{占比87\%}）
\end{itemize}

\textbf{并行损失细分（推测）：}

\begin{enumerate}
    \item \textbf{算法本身开销（最大瓶颈）}：$\sim$40\%
    \begin{itemize}
        \item RecursiveDoubling单线程：117.72ms
        \item Sequential单线程：46.98ms
        \item 额外计算：117.72 - 46.98 = 70.74ms（2.5×慢）
        \item 分摊到16线程：70.74 / 16 = 4.42ms
    \end{itemize}
    
    \item \textbf{内存访问模式灾难（次要瓶颈）}：$\sim$35\%
    \begin{itemize}
        \item Sequential：顺序访问，缓存命中率>95\%
        \item RecursiveDoubling：跳跃访问（stride=$2^k$），缓存命中率<50\%
        \item 估计额外开销：$\sim$8ms
    \end{itemize}
    
    \item \textbf{对数级同步开销}：$\sim$10\%
    \begin{itemize}
        \item 同步次数：$\log_2(4M) \approx 22$次
        \item 每次barrier：$\sim$0.02ms
        \item 总计：22 × 0.02 ≈ 0.44ms（相对较小）
    \end{itemize}
    
    \item \textbf{线程创建与调度}：$\sim$5\%
    \item \textbf{负载不均衡与其他}：$\sim$10\%
\end{enumerate}

\textbf{与Gauss-Seidel的对比：}

\begin{itemize}
    \item \textbf{Gauss-Seidel主要瓶颈}：同步开销（小规模90\%+）或内存带宽（大规模）
    \item \textbf{三对角求解主要瓶颈}：算法本身开销（40\%）+ 内存访问模式（35\%）
    \item \textbf{关键差异}：三对角的算法本身太快（$O(n)$），打破依赖链的代价相对巨大
\end{itemize}

这充分说明：\textbf{不是所有算法都适合并行化}，当串行算法已经极优（$O(n)$复杂度）时，并行化的收益极其有限。
